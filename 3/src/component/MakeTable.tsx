...
const MakeTable = (props: { data: detailType[][]; category: string[]; updater: (data: detailType[][]) => void; index: number; updater2: (data: string[]) => void }) => {
  const plusTable = () => {
      ...
      <table className="tg">
        ...
        <tbody>
          {props.data[props.index].map((data2: detailType, index: number) => (
            <MakeItem data={props.data} key={index} index={index} updater={props.updater} tableCount={props.index} />
          ))}
          <tr>
            <td className="tg-0lax" colSpan={6}>
              <button type="button" onClick={plusTable}>
                +
              </button>
            </td>
          </tr>
